...
def tldashboard():
    if 'loggedad' in session:
        ...
        cur.close()
        cur = mysql.connection.cursor(MySQLdb.cursors.DictCursor)
        cur.execute("select applyleaves.id as lid, employee.empname, employee.empId, employee.dep, employee.id, applyleaves.levtype, applyleaves.postingdate, applyleaves.status from applyleaves join employee on applyleaves.empid=employee.id where employee.dep='IT' order by lid desc limit 6")
        val = cur.fetchall()
        count = 1
